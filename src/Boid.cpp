...
Pvector Boid::Separation(const vector<Boid>& boids)
{
    ...
    for (int i = 0; i < boids.size(); i++) {
        ...
        float d = this->location.distance(boids[i].location);
        // If this is a fellow boid and it's too close, move away from it
        if ((d > 0) && (d < desiredseparation)) {
            Pvector diff(0,0);
            diff = diff.subTwoVector(this->location, boids[i].location);
            diff.normalize();
        ...
        if ((d > 0) && (d < desiredseparation) && predator == true
            ...
            pred2pred = pred2pred.subTwoVector(this->location, boids[i].location);
            pred2pred.normalize();
        ...
        // a predator, then create a large separation Pvector
            ...
            Pvector pred(0, 0);
            pred = pred.subTwoVector(this->location, boids[i].location);
            pred.mulScalar(900);
